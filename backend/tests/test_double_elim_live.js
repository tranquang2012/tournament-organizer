...
const TOUR_ID = process.env.TEST_TOUR_ID_DOUBLE_ELIM;
...
async function run() {
  ...
  while (true) {
    ...
    roundCount++;
    if (roundCount > 15) {
      console.log('Safety break: Exceeded 15 round loops.');
      break;
    }
  }
...
run().then(() => {
  console.log('\nDouble elimination live progression simulation successfully completed!');
  process.exit(0);
}).catch(err => {
